...
if opt.gzsl:
    print('EPOCH          |  D_cost  |  G_cost  |  F_cost  |  R_cost  |  ACC_unseen  |  ACC_seen  |    H    |')
else:
    print('EPOCH          |  D_cost  |  G_cost  |  F_cost  |  R_cost  |  ACC_unseen  |')
...
#------------------------------------------------------------------------------#
        ...
        for iter_d in range(opt.critic_iter):
            ...
            # Gradient Penalty
            gradient_penalty = calc_gradient_penalty(netD, input_res, fake_vf_v.data, input_att_v)
...
#------------------------------------------------------------------------------#
        ...
        # visual feature for visualization
...
#------------------------------------------------------------------------------#
        ...
        # train_hf_v = netF(input_vf_v)
        gen_hf_v = netF(gen_vf_v)

        # hidden feature for visualization
        # train_hf = train_hf_v.data
...
#------------------------------------------------------------------------------#
    ...
    if opt.gzsl:
        ...
        print('[{:^4d}/{:^4d}]    |{:^10.4f}|{:^10.4f}|{:^10.4f}|{:^10.4f}|{:^14.4f}|{:^12.4f}|{:^9.4f}|'.format(epoch+1, opt.nepoch, D_cost.data[0], G_cost.data[0], F_cost.data[0], R_cost.data[0], cls_.acc_unseen, cls_.acc_seen, cls_.H))
        ...
        if cls_.H > max_H:
            ...
            # hf_train_embed = TSNE(n_components=3).fit_transform(train_hf.cpu().numpy())
        ...
        print('[{:^4d}/{:^4d}]    |{:^10.4f}|{:^10.4f}|{:^10.4f}|{:^10.4f}|{:^14.4f}|'.format(epoch+1, opt.nepoch, D_cost.data[0], G_cost.data[0], F_cost.data[0], R_cost.data[0], acc))
...
np.save(file=root+'/vf_train_embed', arr=vf_train_embed) # 训练的vf
np.save(file=root+'/vf_gen_embed', arr=vf_gen_embed) # 生成vf
# np.save(file=root+'/hf_train_embed', arr=hf_train_embed) # 训练vf得到的hf
np.save(file=root+'/hf_gen_embed', arr=hf_gen_embed) # 生成vf得到的hf