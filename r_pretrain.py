...
# r_criterion = nn.PairwiseDistance(p=2)
...
for iter_r in range(opt.r_iteration):
    ...
    for i in range(0, data.ntrain, opt.batch_size):
        ...
        pretrain_att = netR(input_resv) # pretrain_att是Variable类型

        pretrain_r_loss = r_criterion(pretrain_att, input_attv)
        pretrain_r_loss = pretrain_r_loss.mean()
        pretrain_r_loss.backward(mone)

        optimizerR.step()

        print("iter:[%d/%d] batch: %d r_loss:%f" % (opt.r_iteration, iter_r, batch_num, pretrain_r_loss.data))
