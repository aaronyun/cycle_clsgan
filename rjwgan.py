...
for epoch in range(opt.nepoch):
    for i in range(0, data.ntrain, opt.batch_size):
        ...
        for iter_d in range(opt.critic_iter):
            ...
            Wasserstein_D = criticD_real - criticD_fake #! Increase
            D_cost = criticD_fake - criticD_real + gradient_penalty #! Decreases
            optimizerD.step()
        ...
        # caculate r loss with another neural network
        att_consistency = netJ(syn_att, input_attv)
        R_cost = att_consistency.mean()

        # update R and J simultaneously
        R_cost.backward()
        optimizerJ.step()
        optimizerR.step()
