...
def attack_Linf_PGD(input_vf_v, ones, input_att_v, input_label_v, netD, d_loss, steps, epsilon):
    netD.eval()

    adverse_vf = input_vf_v.data.clone()
    adverse_vf_v = Variable(adverse_vf, requires_grad=True)
    optimizer = Linf_SGD([adverse_vf_v], lr=0.0078)

    for _ in range(steps):
        optimizer.zero_grad()
        netD.zero_grad()

        d_bin, d_multi = netD(adverse_vf_v, input_att_v)
        loss = -d_loss(d_bin, ones, d_multi, input_label_v, lam=0.5)
        loss.backward()
        #print(loss.data[0])
        optimizer.step()

        diff = adverse_vf_v.data - input_vf_v.data
        diff.clamp_(-epsilon, epsilon)
        adverse_vf_v.data.copy_((diff + input_vf_v.data).clamp_(-1, 1))
